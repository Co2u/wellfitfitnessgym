...
const ListOfInventoryItems = () => {
    ...
    const [editForm, setEditForm] = useState({
        ...
        inventoryImage: "",
        inventoryId: "",
        inventoryPrice: "" // Add price field
    });
    ...
    const columns = [
        ...
        {
            name: 'Price',
            selector: row => `₱ ${row.inventory_price}`, // Add price column
            sortable: true,
        },
    ...
    const handleEditItem = async (e) => {
        ...
        formData.append("inventory_price", editForm.inventoryPrice); // Add price to form
    ...
    const handleSelectToEditItem = (id) => {
        ...
        if (selectedItem) {
            setEditForm({
                ...
                inventoryPrice: selectedItem.inventory_price || "" // Set price
            });
        }
    };
  ...
  const filteredData = get_all_inventory_items.filter(item => {
    ...
    const inventoryPrice = (item.inventory_price || "").toString().toLowerCase(); // Add price to filter
    return (
        ...
        inventoryPrice.includes(searchQuery.toLowerCase())
    );
  });
          ...
          <div className="card-body">
            <h5 className="card-title">Edit Item</h5>
                ...
                <form onSubmit={handleEditItem} encType="multipart/form-data">
                    ...
                    <div className="row mb-3">
                        <label htmlFor="inventoryPrice" className="col-sm-2 col-form-label">Price</label>
                        <div className="col-sm-10">
                            <input
                                type="number"
                                id="inventoryPrice"
                                name="inventoryPrice"
                                className="form-control"
                                value={editForm.inventoryPrice}
                                onChange={(e) => setEditForm({ ...editForm, inventoryPrice: e.target.value })}
                            />
                        </div>
                    </div>
